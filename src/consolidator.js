...
import { deduplicate, findExactDuplicates } from './deduplicator.js';
import { normalizeDates } from './normalizer.js';
import { pruneEntries, trimToMaxLines } from './pruner.js';
import { countLines, ensureDir, formatDate, getDailyFiles, readFileSafe, safePath, timestamp } from './utils.js';
 ...
 */
export function consolidate(workspacePath, options = {}) {
  ...
  if (!dryRun) {
    ...
    fs.writeFileSync(path.join(reportDirPath, reportName), `${JSON.stringify(report, null, 2)}\n`, 'utf-8');
 ...
 */
export function buildMemoryIndex(entries, categories, _currentDate, report) {
  const lines = [];

  lines.push('# Long-Term Memory');
  lines.push(
    `<!-- Last consolidated: ${timestamp()} | Files processed: ${report.phase1.newFileCount} | Entries: ${entries.length} -->`,
  );
  lines.push('');
  ...
  for (const cat of categories) {
    ...
    for (const entry of catEntries) {
      ...
      const text = entry.text.length > 200 ? `${entry.text.substring(0, 197)}...` : entry.text;
 ...
 */
export function getStats(workspacePath) {
  const config = loadConfig(workspacePath);
  const memoryDirPath = safePath(workspacePath, config.memoryDir);
  const memoryIndexPath = safePath(workspacePath, config.memoryIndex);
