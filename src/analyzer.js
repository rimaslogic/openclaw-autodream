import { extractBullets, extractSections } from './utils.js';
 ...
 */
export function analyzeFile(content, date, source) {
  if (!content) return [];
  ...
  if (firstHeaderIdx > 1) {
    const preamble = lines
      .slice(1, firstHeaderIdx === -1 ? undefined : firstHeaderIdx)
      .join('\n')
      .trim();
    if (preamble) {
 ...
 */
export function scoreImportance(text) {
  let score = 5;
 ...
 */
export function extractPeople(entries) {
  const people = new Map();

  for (const entry of entries) {
    const namePattern = /\*\*([A-Z][a-z]+(?:\s[A-Z][a-z]+)*)\*\*/g;
    for (const match of entry.text.matchAll(namePattern)) {
      const name = match[1];
      if (/^(Note|Warning|Important|Critical|TODO|TIP|Example|Context|Summary|Key|Status|Update|Result)$/i.test(name))
        continue;
      if (!people.has(name)) {
 ...
 */
export function extractProjects(entries) {
  const projects = new Map();

  for (const entry of entries) {
    const projPattern = /\*\*([A-Z][a-zA-Z0-9\s-]+)\*\*/g;
    for (const match of entry.text.matchAll(projPattern)) {
      const name = match[1].trim();
      if (name.length > 30 || name.length < 2) continue;
      if (
        /^(Note|Warning|Important|Critical|TODO|TIP|Key|Status|Update|Result|Current|Target|Decision|Strengths?|Weaknesses?)$/i.test(
          name,
        )
      )
        continue;
